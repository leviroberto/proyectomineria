...
public class DesktopNotify {
    ...
    public static final int TIPO_CLICKEADO = 9;
    public static final int TIPO_SINCLICK = 10;
    public static final int TIPO_CUMPLEAÑOS = 11;


    public static final Image[] defaultIcons;

    static {
        defaultIcons = new Image[11];
        for (int i = 0; i < defaultIcons.length; i++) {
            defaultIcons[i] = new ImageIcon(DesktopNotify.class.getResource("/SistemaLara/capa5_imagenes/" + (i + 1) + ".png")).getImage();
        }
    }
    ...
    public static void showDesktopMessage(String title, String message, int type) {
        showDesktopMessage("EMPRESA " + IniciarSesion.getUsuario().getEmpresa().getNombreComercial() + " DICE : ", message, type, null, null, 0);
    }

    public static void mostraMernsajeSinTiempo(String title, String message, int type) {
        mostraMernsajeSinTiempo("EMPRESA " + IniciarSesion.getUsuario().getEmpresa().getNombreComercial() + " DICE : ", message, type, null, null, 0);
    ...
    public static void showDesktopMessage(String title, String message, int type, ActionListener action) {
        mostraMernsajeSinTiempo(title, message, type, null, action, 0);
    }

    //PARA EL CARD QUE SE HA SELECCIONADO
    /**
     * Creates and shows a new notification. If there isn't an instance of the
     * DesktopNotifyDriver thread running, it will be created and started to
     * show your notification window.
...
//        pane.setTimeout(maxTimeMillis);
    ...
    public static void mostraMernsajeSinTiempo(String title, String message, int type, Image icon, ActionListener action, long maxTimeMillis) {
        DesktopNotify pane = new DesktopNotify(title, message, type, icon);
        pane.setTimeout(maxTimeMillis);
        //   pane.setTimeout(5000);
        pane.setAction(action);
    ...
    public static void eliminarWindowsAll() {
        DesktopNotifyDriver.eliminarWindowsAll();
    }
