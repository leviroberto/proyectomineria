 ...
 */
public class FCSafePropertySetter<T> extends TimingTargetAdapter {
    private final AtomicReference<KeyFrames<T>> keyFrames;
    private final boolean isToAnimation;
    private final Getter<T> getter;
    private final Setter<T> setter;

    protected FCSafePropertySetter(KeyFrames<T> keyFrames, boolean isToAnimation, Getter<T> getter, Setter<T> setter) {
    ...
    public static <T> TimingTarget getTarget(Setter<T> setter, T... values) {
        return new FCSafePropertySetter<>(new KeyFrames.Builder<T>().addFrames(values).build(), false, null, setter);
    }

    public static <T> TimingTarget getTarget(Setter<T> setter, KeyFrames<T> keyFrames) {
        return new FCSafePropertySetter<>(keyFrames, false, null, setter);
    }
    ...
    public static <T> TimingTarget getTargetTo(Getter<T> getter, Setter<T> setter, KeyFrames<T> keyFrames) {
        return new FCSafePropertySetter<>(keyFrames, true, getter, setter);
    }
